...
if [ -z "$targets" ]; then
  ...
  $HDC_BIN -t "$target_id" shell aa force-stop com.arialentropy.kuiklycalendar
  $HDC_BIN -t "$target_id" install entry/build/default/outputs/default/entry-default-signed.hap
  $HDC_BIN -t "$target_id" shell aa start -a EntryAbility -b com.arialentropy.kuiklycalendar
  done
fi